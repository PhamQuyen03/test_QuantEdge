$(document).ready(function () {
    var myTimer = setInterval(random, 800);
    function random() {
        var lengthArr = document.getElementById('sizeList').textContent;

        for (var i = 0; i < lengthArr; i++) {
            var num = Math.floor(Math.random() * (50 - (-50) + 1) + (-50));
            var num1 = Math.floor(Math.random() * (50 - (-50) + 1) + (-50));
            var sell = document.getElementById("sell" + i.toString()).textContent;
            var valueSell = parseFloat(sell).toFixed(4);
